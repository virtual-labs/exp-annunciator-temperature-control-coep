...
example.View = draw2d.Canvas.extend({
	...
	init:function(id, numOfWindows, alarmtype, ISASeq, appId, maxLimit, minLimit){
		this._super(id);
    ...
    {   
       ...
       numOfWindows = ExpTrackData.numOfWindows;
       alarmtype = ExpTrackData.alarmtype;
       ISASeq = ExpTrackData.ISASeq;
       appId = ExpTrackData.appId;
    	
    	
	    var figure;
        var type = $(droppedDomNode).data("shape");
		
        //alarm annunciator Symbols
        if(type == "ANDGate"){
			
        	ANDGate = andGateSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = ANDGate;
	//		$("#ANDGate").draggable({ disabled: true });
		}
        //alarm annunciator Symbols OR Gate
        if(type == "ORGate"){
			
        	ORGate = orGateSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = ORGate;

		}
        
        //alarm annunciator Symbols BULB Symbol
        if(type == "BULBSymbol"){
			
        	BulbWindow = bulbWindowSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = BulbWindow;

		}
          if(type == "JKFF"){
			
        	JKFF = KJFFSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = JKFF;

		}
        
        if(type == "MultiVibrator"){
			
        	MultiVibrator = MultiVibratorSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = MultiVibrator;

		}
        
        if(type == "MUX"){
			
        	MUX = MUXSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = MUX;

		}
       if(type == "TESTInput"){
			
    	   TESTInput = TESTInputSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = TESTInput;

		}
       if(type == "ACKInput"){
			
    	   ACKInput = ACKInputSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = ACKInput;

		}
       if(type == "RESETInput"){
			
    	   RESETInput = RESETInputSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = RESETInput;

		}
       if(type == "ALARM"){
			
    	   ALARM = AlarmSymbol(this,x,y,numOfWindows, alarmtype, ISASeq, appId);
			figure = ALARM;
